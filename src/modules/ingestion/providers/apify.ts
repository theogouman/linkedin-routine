...
export class ApifyIngestionProvider implements IngestionProvider {
  ...
  constructor(options: ApifyProviderOptions = {}) {
    ...
    this.profileActor = options.profileActor ?? readEnv("APIFY_PROFILE_ACTOR") ?? null;
  ...
  async fetchProfile(profileUrl: string): Promise<FetchedProfile | null> {
    if (!this.profileActor) return null;
    try {
      ...
      const photo = raw.photo ?? raw.avatar ?? raw.pictureUrl;
      return {
        profileUrl,
        publicIdentifier: stringOrNull(raw.publicIdentifier),
        name:
        ...
        avatarUrl:
          typeof photo === "string"
            ? photo
            : stringOrNull((photo as Record<string, unknown> | undefined)?.url),
      };
    } catch {
      // L'enrichissement est un confort : son échec ne doit pas empêcher
      // l'ajout du compte, dont les métadonnées seront déduites du 1er post.
      return null;
    }
  }
}
